...
namespace StreamingPlayer
{
	...
	public class PlaylistModule : NativeModule
	{
		...
		{
			if (CanCallBackToJS)
				_statusChanged.RaiseAsync(_statusChanged.ThreadWorker, status.Stringify());
		}

		void OnHasNextChanged(bool n)
		{
			if (CanCallBackToJS)
				_hasNextChanged.RaiseAsync(_hasNextChanged.ThreadWorker, n);
		}

		void OnHasPreviousChanged(bool p)
		{
			if (CanCallBackToJS)
				_hasPreviousChanged.RaiseAsync(_hasPreviousChanged.ThreadWorker, p);
		}

		void OnCurrentTrackChanged()
		{
			if (CanCallBackToJS) {
				_currentTrackChanged.RaiseAsync(_currentTrackChanged.ThreadWorker);
			}
		}
